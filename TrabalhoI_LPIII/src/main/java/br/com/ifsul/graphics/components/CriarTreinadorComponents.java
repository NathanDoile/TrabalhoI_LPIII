...
public abstract class CriarTreinadorComponents {
    ...
    public static JButton proximaConversaButton(JLabel conversa,
                                                ...
                                                TreinadorController controller,
                                                JFrame conversaUI,
        ...
        button.setCursor(new Cursor(Cursor.HAND_CURSOR));
        button.addActionListener(e -> proximaConversa(conversa, conversaList, controller, conversaUI, telaInicialUI));
        return button;
    }
        ...
        for(int i=0;i<conversaList.size();i++) {
            ...
            if(conversaList.get(i).equals(conversa.getText())) {
                if(i+1 == conversaList.size()) {
                    iniciarNomearTreinador(controller, conversaUI, telaInicialUI);
                }
                else {
    ...
    private static void iniciarNomearTreinador(TreinadorController controller,
                                               JFrame conversa,
                                               JFrame telaInicial) {
        new UICriarTreinador(controller, telaInicial);
