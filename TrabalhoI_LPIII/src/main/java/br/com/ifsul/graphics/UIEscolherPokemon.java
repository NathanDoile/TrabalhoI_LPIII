...
import static br.com.ifsul.graphics.assets.BatalhaPokemonAssets.getPokemonAliadoSprite;
import static br.com.ifsul.graphics.assets.PokeHubAssets.getIcon;

public class UIEscolherPokemon extends JFrame{

	private DefaultListModel<String> dadosLista = new DefaultListModel<>();
	private JList<String> lista = new JList<>(dadosLista);

	public UIEscolherPokemon(PokemonController pokemonController,
                             Treinador treinador,
							 JLabel spriteAliado,
							 JLabel nomeAliado,
							 JLabel mensagem,
		...
		lista.addListSelectionListener(evt -> trocarPokemon(
				spriteAliado,
				nomeAliado,
				mensagem,
				lvlAliado,
	...
	private void trocarPokemon(
		...
		String text = lista.getSelectedValue();
		int index = text.indexOf(".");
		String id = text.substring(9, index);
		Pokemon pokemonAliado = controller.buscarPokemonService.porId(Long.valueOf(id));

		spriteAliado.setIcon(new ImageIcon(getPokemonAliadoSprite(pokemonAliado).getScaledInstance(spriteAliado.getWidth(), spriteAliado.getHeight(), Image.SCALE_SMOOTH)));
		nomeAliado.setText(pokemonAliado.getNome().toString());
		mensagem.setText("Lvl. "+pokemonAliado.getLevel());
		lvlAliado.setText(pokemonAliado.getNome().toString()+", eu escolho voce!");
